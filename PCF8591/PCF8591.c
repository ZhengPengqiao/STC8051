 ...
 *  返回    ：  得到ADC的值
 ******************************************************************************/
unsigned char getADCAutoValue(ADMODE mode, unsigned char chan)
{
	unsigned char val;
	i2cStart();
	if( !i2cWrite(0x48 << 1) )  //寻址PCF8591,如未应答,则停止操作,并返回0
	...
	switch(mode)
	{
		case SINGLE0: i2cWrite(0x44 | chan);   break;
		case DIFF1:   i2cWrite(0x54 | chan);   break;
		case DIFF2:   i2cWrite(0x64 | chan);   break;
		case DIFF3:   i2cWrite(0x74 | chan);   break;
	}
